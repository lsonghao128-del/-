...
const shareMetric = document.getElementById("share-metric");
const shareDrinkViewBar = document.getElementById("share-drink-view-bar");
const shareChart = document.getElementById("share-chart");
const shareTotal = document.getElementById("share-total");
const shareTotalLabel = document.getElementById("share-total-label");
...
const shareMetrics = {
  sales: { label: "销售占比", valueLabel: "销量", totalLabel: "总销量", format: (value) => fmtInt.format(value) },
  netRevenue: { label: "实际收款金额占比", valueLabel: "实收金额", totalLabel: "实收金额", format: (value) => fmtMoney.format(value) },
  grossOrder: { label: "原单金额占比", valueLabel: "原单金额", totalLabel: "原单金额", format: (value) => fmtMoney.format(value) },
  tax: { label: "税额占比", valueLabel: "税额", totalLabel: "税额", format: (value) => fmtMoney.format(value) },
};
...
function setMode(mode) {
  ...
  document.getElementById("share-view").hidden = mode !== "share";
...
function buildDrinkSummaryDataset() {
  const items = buildDrinkSummaryItems(DATA.total);
  return buildDatasetFromItems("饮品（商品汇总）", items);
}

function buildDrinkSummaryItems(sourceItems) {
...
function renderShareView() {
  const metric = shareMetrics[state.shareMetric];
  const rows = buildShareRows();
  const total = rows.reduce((sum, item) => sum + item.shareValue, 0);
  const maxValue = rows[0]?.shareValue || 0;

  shareDrinkViewBar.hidden = state.shareCategory !== "饮品";
  document.querySelectorAll(".share-drink-button").forEach((button) => {
    button.classList.toggle("active", button.dataset.shareDrinkView === state.shareDrinkView);
  });

  shareTotal.textContent = metric.format(total);
  shareTotalLabel.textContent = metric.totalLabel;
  shareCount.textContent = fmtInt.format(rows.length);
  shareTop.textContent = total ? fmtPct.format((rows[0]?.shareValue || 0) / total) : "0.0%";
  ...
  shareChart.innerHTML = `
    <div class="share-list-head">
      ...
      <span>商品 / 规格</span>
      <span>占比</span>
      <span>${escapeHtml(metric.valueLabel)}</span>
      <span>比例</span>
    </div>
    ${rows.map((item, index) => {
    const share = total ? item.shareValue / total : 0;
    const width = maxValue ? Math.max(2, (item.shareValue / maxValue) * 100) : 0;
    return `
      <div class="share-bar-row">
        <div class="share-rank">${index + 1}</div>
        <div class="share-category">${escapeHtml(item.category)}</div>
        <div class="share-name">
          <strong>${escapeHtml(item.nameCn)}</strong>
          <span>
            ${escapeHtml(item.nameEn)}${item.spec ? ` / ${escapeHtml(item.spec)}` : ""}
            ${item.specList ? ` / ${escapeHtml(item.specList)}` : ""}
          </span>
        </div>
        <div class="share-bar-track">
          <div class="share-bar-fill" style="width:${width}%"></div>
        </div>
        <div class="share-value">${metric.format(item.shareValue)}</div>
        <div class="share-percent">${fmtPct.format(share)}</div>
      </div>
    `;
  }).join("")}
  `;
...
function buildShareRows() {
  const category = state.shareCategory;
  let rows;
  if (category === "饮品" && state.shareDrinkView === "summary") {
    rows = buildDrinkSummaryItems(DATA.total);
  } else {
    rows = DATA.total.filter((item) => category === "all" || item.category === category);
  }

  return rows
    .map((item) => ({
      ...item,
      shareValue: Number(item[state.shareMetric]) || 0,
    }))
    .sort((a, b) => b.shareValue - a.shareValue || a.category.localeCompare(b.category) || a.nameCn.localeCompare(b.nameCn));
}
...
document.querySelectorAll(".share-drink-button").forEach((button) => {
  button.addEventListener("click", () => {
    state.shareDrinkView = button.dataset.shareDrinkView;
    renderShareView();
  });
});
...
shareCategory.addEventListener("change", () => {
  state.shareCategory = shareCategory.value;
  renderShareView();
});
shareMetric.addEventListener("change", () => {
  state.shareMetric = shareMetric.value;
  renderShareView();
});
